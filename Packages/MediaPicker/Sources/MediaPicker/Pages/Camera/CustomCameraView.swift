...
struct CustomCameraView<CameraViewContent: View>: View {
    ...
    public typealias CameraViewHandler = ((LiveCameraView, @escaping DefaultHandler, @escaping DefaultHandler, @escaping DefaultHandler, @escaping DefaultHandler, @escaping DefaultHandler, @escaping DefaultHandler) -> CameraViewContent)

    // params
    @ObservedObject var viewModel: MediaPickerViewModel
    let didTakePicture: () -> Void
    let didPressCancel: () -> Void
    var cameraViewBuilder: CameraViewHandler
            ...
            VStack(spacing: 10) {
                if cameraSelectionService.hasSelected {
                    HStack {
                        ...
                        Spacer()
                        if selectionParamsHolder.mediaType.isVideoAllowed {
                            photoVideoToggle
                        }
                        Spacer()
                ...
                else if selectionParamsHolder.mediaType.isVideoAllowed {
                    photoVideoToggle
                }
